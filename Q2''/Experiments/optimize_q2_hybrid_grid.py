# -*- coding: utf-8 -*-
"""v2 load + 原版 pv 的最终网格：beta/window/M/kappa"""
...
def run(beta,window,M=20,kappa2=0.132917,seed=20260101):
...
def scen(window,M):
    k=(window,M)
    if k not in cache:
        cache[k]=run(0.0,window,M)  # placeholder not used; rebuild inline in run
    return None

# run inline (scenario rebuilt each call; acceptable)
variants=[]
for beta,window,M,kappa in [
    (0.0,30,20,0.132917),(0.005,30,20,0.132917),(0.01,30,20,0.132917),
    (0.02,30,20,0.132917),(0.03,30,20,0.132917),(0.01,45,20,0.132917),
    (0.01,60,20,0.132917),(0.01,30,30,0.132917),(0.01,30,20,0.0),(0.01,30,20,0.531667),
]:
    a=run(beta,window,M,kappa); a.update(variant=f"b{beta}_w{window}_M{M}_k{kappa}"); variants.append(a); print("variant",a["variant"],flush=True)
json.dump(variants,open(os.path.join(GS,"_opt_hybrid_grid.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=2)
print("DONE")
