# -*- coding: utf-8 -*-
"""q4_core.py —— 问题 4-2 单日两阶段随机 LP + CVaR 求解器（波动电价版）。

【与 Q2 的关系】本模块是 `All_Code/Q2_yy/.../optimizer.py::solve_day2` 的**严格推广**：
变量布局、约束集合、罚项、终端处理、残差验算全部逐行保持一致，
**唯一改动**是把确定电价向量 `price (T,)` 换成逐场景价格矩阵 `P (M,T)`：

    Q2 ： C = Σ_t π_t·x_t + 5·Σ_t π_t·e_t                     （π 已知常数）
    Q4-2： C^ω = Σ_t λ^ω_t·x_t + 5·Σ_t λ^ω_t·e^ω_t            （λ 逐日波动且决策时未知）

对应三处系数改写（其余不变）：
  1. 目标中 x 的期望成本系数 = (1−β)·Σ_m prob·λ^m_t = (1−β)·E[λ_t]；
  2. 目标中 e 的费用系数 = (1−β)·prob·5·λ^m_t（逐场景）；
  3. CVaR 线性化约束 q_m ≥ C^ω_m − ζ 内的 λ 系数逐场景取值。

【题意】
  - 每天 0:00 一次性决定计划购电量 x 与充放电 c/r（第一阶段，跨场景共享，非预期性）。
  - 供电不足时紧急购电，电价 = 交易时刻电价的 5 倍。
  - 除紧急购电外，其他时段购电费用按**计划购电量**计算（无论是否提取）。
  - 提取量不得超过计划量：y ≤ x。
  - 目标：min (1−β)·E[C] + β·CVaR_α(C) + κ·|终端 SOC 偏差|。

【口径】`others/Q4重现Q2的口径说明.md`。索引 t=0..143 对应物理区间 0:00-0:10 … 23:50-24:00。
"""
