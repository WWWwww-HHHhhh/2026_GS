...
def main():
    daily=pd.read_csv(ROOT/"Sall"/"daily.csv")
    slots=pd.read_csv(ROOT/"Sall"/"intervals.csv")
    book=openpyxl.load_workbook(ROOT/"result3.xlsx",read_only=False,data_only=True)
    ...
    if emergency.max_row!=3819:
        raise AssertionError(f"emergency event count mismatch: {emergency.max_row-1}")
    print(f"PASS result3.xlsx: 334 dates, 96,192 purchase cells, 2,004 storage rows, "
