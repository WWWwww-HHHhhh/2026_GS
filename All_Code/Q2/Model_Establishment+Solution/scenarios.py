...
class ScenarioEngine:
    ...
    def _available_blocks(self, i: int, blocks: list) -> list:
        """返回决策日 i 可用的残差块（严格早于 i 天；2025-01-01 即 day0 除外）。"""
        return [b for b in blocks if 1 <= b.day < i]
...
def run_scenarios(M_demo: int = 20) -> dict:
    ...
    print("    因果检查：决策日 i 的残差库只含日期 < i（2025-01-01 不入库）")

    eng.save_demo(L_all, G_all, "2025-07-15")

    # 缓存场景供 rolling 复用（M=20 默认）
    cache = {"M": M_demo, "L_all": L_all, "G_all": G_all}
