...
【产出】Q4_WYH/result4-2_附件3口径.xlsx（在官方模板副本上填写，全部输出留在 Q4_WYH）
...
WYH = Path(__file__).resolve().parent
...
OUT_XLSX = NEW / "result4-2_附件3口径.xlsx"
...
def verify() -> None:
    ...
    (NEW / "export_verify_附件3口径.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    for name, good, val in rows:
