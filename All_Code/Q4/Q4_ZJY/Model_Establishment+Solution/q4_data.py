"""Q4-3 波动电价接入：读附件 4 的每日波动电价，替换 Q3 中附件 1 的固定电价。

附件 4 结构：行 = 日期（365 天，2025-01-01 起），列 = 144 个 10 分钟时段
（右端点标签 00:10 … 0:00+1），值 = 该日该时段电价（元/kWh，每天不同）。
转置后 price[t, day] = 第 day 天第 t 个时段的电价，与 Q3 的 price 形状 (T=144, D=365) 一致。

信息假设（论文须写明）：附件 4 视为**日前发布电价**，每天 0:00 制定计划时
当日 144 时段电价已知，故价格向量为确定量，不构成未来信息泄露。
"""
