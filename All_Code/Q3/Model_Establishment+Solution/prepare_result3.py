...
def main() -> None:
    ...
    if report["status"]!="PASS" or not any(x["strategy"]=="Sall" for x in report["strategies"]):
        raise RuntimeError("Sall must pass independent validation before result3 export")
    slots=pd.read_csv(args.folder/"Sall"/"intervals.csv")
    daily=pd.read_csv(args.folder/"Sall"/"daily.csv")
