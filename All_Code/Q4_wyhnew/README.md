# Q4_wyhnew：问题四第二问的唯一正式工作区（自包含可运行）
...
原有的 `All_Code/Q4/Q4_wyh` 保留为历史与对照，不再作为论文取数来源。
...
| 目录 | 定位 |
| --- | --- |
| `All_Code/Q4_wyhnew`（本目录） | 问题四第二问的正式工作区，论文取数来源 |
| `All_Code/Q4/Q4_wyh` | 历史工作区，保留旧口径结果与全部过程记录，不再取数 |
