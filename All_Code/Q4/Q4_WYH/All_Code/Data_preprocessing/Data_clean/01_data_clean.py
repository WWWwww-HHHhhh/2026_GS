...
01_data_clean.py
================
数学建模数据清洗检查脚本（只读原始 xlsx，输出清洗后的 CSV 与检查报告）。

处理规则（与建模口径一致）：
1. 附件3 的“日期”列因 Excel 合并单元格出现 1095 个 NaN，属于结构性占位，
   按约定向下填充（ffill），不视为真实缺失，不参与“缺失值不填充”规则。
2. 负荷/光伏合理量级上界统一为 [0, 12000] kW，超过上界只报告、不修改；
   与储能 12000 kWh 参数严格区分（储能参数仅作为模型约束，不进入清洗阈值）。
3. 光伏夜间窗口取 19:30 - 05:30，严格 > 0 才报告，不给容差。
4. 附件1 没有日期列，查重仅按“时间(时段)”，且不参与 2025 全年 365 天连续性检查。
5. 物理不可能值（负负荷、负光伏）：报告后截断为 0。
6. 重复记录：保留第一条，记录删除条数。
7. 缺失值：只报告，不自动填充。
8. 其他可疑点（电价<=0、上界超限、光伏夜间非零）：只报告，不删除、不修改。
9. 不修改原始 xlsx；不凭空填充缺失；不读取未来数据。
"""
