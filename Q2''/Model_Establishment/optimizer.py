# -*- coding: utf-8 -*-
"""
Q2 两阶段随机 LP + CVaR 单日优化器（2026 数模 C 题 问题二，最终版）
============================================================
...
if __name__ == "__main__":
    import pickle, os
    import numpy as np
    Q2P = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    ...
    with open(os.path.join(DATA_PROC, "forecasts.pkl"), "rb") as fh:
        fc = pickle.load(fh)
    # 用最终参数自检报告期第 1 天 2025-02-01（索引 31），场景退化为点预测即可自检
    i = 31
    price = ds["price"][:, i]
    L = np.clip(fc["Lhat"][:, i][None, :], 0, None)
    G = np.clip(fc["Ghat"][:, i][None, :], 0, None)
    check_day(price, {"L": L, "G": G}, 6000.0, {"kappa2": 0.531667, "beta": 0.01})
