...
OUT = r"D:\Personal\Desktop\问题二论文.docx"
...
para("自动校验 12 项全部通过：功率转电量乘 1/6 一致；结算平衡残差最大值 3.411×10^{−13}；SOC 始终在 [1200, 10800]；充放电不超过 833.333 kWh；跨日 SOC 传递一致；因果性成立（特征与残差库均早于决策日）；第一阶段变量跨场景共享；弃光 w = G − g 且非负；紧急购电 5 倍电价复算一致；模板列映射与总量一致；报告费用不含罚项；报告期样本量 334 天。")
