# -*- coding: utf-8 -*-
"""q4_2_rolling.py —— 问题 4-2 全年滚动求解（波动电价下的日前两阶段随机规划）。

【滚动协议（与 Q2_yy 完全一致，仅价格来源不同）】
  1. 预热：2025-01-01 ~ 01-14（day 0..13），参数取基准配置，SOC 从题目给定的 6000 kWh 起连续传递；
  2. 仅历史调参：2025-01-15 ~ 01-31（day 14..30），在 (M, β, κ倍数) 网格上评估；
     选择规则与 Q2 相同 —— 先剔除紧急购电费高于基准配置的候选，再取"调整后总成本"最低者；
     调整后总成本 = 验证期实际总费用 + κ_base·max(0, s_验证起点 − s_验证终点)，防止靠耗空储能虚假降费；
  3. 参数在 01-31 封存，报告期 2025-02-01 ~ 12-31（day 31..364）**只报告、不参与选参**；
     跨日 SOC 从调参末期真实状态继续传递；仅最后一天施加年末 SOC=6000 kWh 硬约束。

【与 Q2 的唯一差别】每天 0:00 求解时，电价不是已知常数，而是(M,T)的场景矩阵；
结算时用当日**实际**波动电价。

运行：
    python q4_2_rolling.py --mode smoke      # 冒烟：只跑 3 天，检查链路
    python q4_2_rolling.py --mode full       # 全流程：预热 + 调参 + 报告期
"""
