...
`Results/Pictures/` 下 6 张图均输出 PDF（投稿，TrueType 嵌字）+ PNG（预览）+ SVG，
文件名自旧版沿用，论文 `\includegraphics` 无需改动。其中两张已换图型：

| 文件 | 现图型 | 说明 |
|---|---|---|
| `q3_fig1_strategy_cost` | 哑铃图（基线点 → 策略点） | 横轴保留绝对费用并放大至差异区间，连线长度即与 Q2 基线的差额。原为截断纵轴的柱状图（纵轴从 14.5 起），会把 0.358 百万元（约 2.4%）的真实差异放大成视觉上的数倍差 |
| `q3_fig3_forecast_conversion_error` | 横向哑铃图（宽扁） | 每行是一个发布时刻，蓝点为线性插值、橙点为阶梯保持，连线长度即两者误差差距，上方标 ×N 倍数。选横向哑铃而非竖向斜率图，是因为宽扁排版下斜率图的折线会被压平、"斜率=倍数"这个读法会失效，而哑铃的行高与图宽无关 |

两张图的渐变连线都用多段短线叠出来（`gradient_seg` 单色稀释、`gradient_seg2` 双色过渡，
双色过渡按 sin 曲线提亮中段，否则蓝→橙在 RGB 里直接插值会掉到灰褐色），
在 PDF/SVG 里仍是纯矢量，未使用 `imshow`，因此不存在放大后糊掉的光栅块。
