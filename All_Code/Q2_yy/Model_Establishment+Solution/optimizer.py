...
def solve_day2(price, scenarios, s0, params=None):
    ...
    result = linprog(c=c_obj, A_ub=A_ub, b_ub=b_ub_arr,
                     A_eq=A_eq, b_eq=b_eq_arr, bounds=bounds, method="highs")
    # SciPy 1.7 所带旧版 HiGHS 偶尔只确认缩放模型最优。依次用不同的
    # HiGHS 算法复核同一个LP；变量、目标和约束均不改变，也绝不接受
    # status=4 的可疑结果。
    if result.status == 4:
        for method, options in (("highs-ds", {"presolve": False}),
                                ("highs-ipm", {}),
                                ("highs-ipm", {"presolve": False})):
            candidate = linprog(c=c_obj, A_ub=A_ub, b_ub=b_ub_arr,
                                A_eq=A_eq, b_eq=b_eq_arr, bounds=bounds,
    ...
    if not np.all(np.isfinite(xopt)) or eq_residual > 1e-5 or ub_violation > 1e-5:
        raise RuntimeError(
            f"LP解未通过独立残差验算: eq={eq_residual:.3e}, ub={ub_violation:.3e}"
        )
...
if __name__ == "__main__":
    ...
    with open(Q2_DATA_PROCESSING / "q2_dataset.pkl", "rb") as fh:
        ds = pickle.load(fh)
    with open(Q2_DATA_PROCESSING / "scenarios_M20.pkl", "rb") as fh:
        sc = pickle.load(fh)
